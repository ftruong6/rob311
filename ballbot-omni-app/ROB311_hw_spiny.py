...
def rad_2_counts(psi_rad):
    ...
    # HERE
    # 64 cpr, 70:1
    psi_counts = psi_rad/(2*np.pi)*70*64
...
if __name__ == "__main__":
    ...
    #Tx = 0
    #Ty = 0
    #note: Tx = 1 actually makes ball rotate about -x, according to right hand rule.
    #same for Ty......
    Tz = 0
    ...
    for t in SoftRealtimeLoop(dt=DT, report=True):
        ...
        Tz+= 3*DT/trialTime   #ramp up in 6 seconds
        if t_now>=trialTime:
            break
        ...
        data = [i] + [t_now] + [theta_x] + [theta_y] + [T1] + [T2] + [T3] + [phi_x] + [phi_y] + [phi_z] + [psi_1] + [psi_2] + [psi_3] + [Tz] + [states["theta_yaw"]]
        dl.appendData(data)
