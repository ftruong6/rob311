...
if __name__ == "__main__":
    ...
    while True:
        ...
        vx,vy = computeVel2((states['theta_roll']-roll)/DT,(states['theta_pitch']-pitch)/DT,(states['psi_1']-psi_1)/DT,(states['psi_2']-psi_2)/DT,(states['psi_3']-psi_3)/DT)
        ...
        vel_kp = 0.1
        #theta_roll_pid.setpoint= vel_kp*vx
        #theta_pitch_pid.setpoint = vel_kp*vy


        Tx = theta_roll_pid(states['theta_roll'])-Theta_KF*np.sin(states['theta_roll'])
        Ty = theta_pitch_pid(states['theta_pitch'])-Theta_KF*np.sin(states['theta_pitch'])
        #Tx = 0
        #Ty = x0
        

        phiCmd=np.array([[-Tx],[-Ty],[0]])
        psiCmd=np.matmul(IK,phiCmd)
        ...
        print('vx ','{:+2.2f}'.format(vx) ,'vy ', '{:+2.2f}'.format(vy))
